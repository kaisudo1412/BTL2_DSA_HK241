...
V xMap<K, V>::put(K key, V value) {
  ...
  if(temp.size() == 0) {  // if the table[index] is empty
    ...
    for(i = 0; i < temp.size(); i++){
      if(keyEqual){
        if(keyEqual(key,temp.get(i)->key)){  // check if key is already allocated
            V old = temp.get(i)->value;
            temp.get(i)->value = value;  
            return old; 
        }
      }

      else if(key == temp.get(i)->key){
            V old = temp.get(i)->value;
            temp.get(i)->value = value;  
            return old; 
      }
...
V& xMap<K, V>::get(K key) {
  ...
  for(int i = 0; i< temp.size(); i++){
      if(keyEqual != 0){
        if(keyEqual(key,temp.get(i)->key)) return temp.get(i)->value;
      }

      else if(temp.get(i)->key == key){
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K)) {
  ...
  // TODO YOUR CODE IS HERE
  for(int i = 0; i< temp.size(); i++){
    if(keyEqual){ // check if keyEqual is referenced
      if(keyEqual(key,temp.get(i)->key)){       
          V backup = temp.get(i)->value;
          if(deleteKeyInMap){ // If deleteKeys is referenced
            deleteKeyInMap(key);
          }
          temp.removeItem(temp.get(i),&deleteEntry);
          count--;
          return backup;
      }
    }
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K),
  ...
  for(int i = 0; i< temp.size(); i++){
      ...
      if(keyEqual !=0 && valueEqual != 0){ // if keyEqual and valueEqual is referenced
        if(keyEqual(key,temp.get(i)->key) && valueEqual(value,temp.get(i)->value)){
          if(deleteKeyInMap){ // If deleteKeyInMap is referenced
            deleteKeyInMap(key);
          }
          if(deleteValueInMap){ // If deleteValueInMap is referenced
      ...
      else if(keyEqual ==0 && valueEqual != 0){ // if keyEqual is not but valueEqual is referenced
          if(key == temp.get(i)->key && valueEqual(value,temp.get(i)->value)){
            if(deleteKeyInMap){ // If deleteKeyInMap is referenced
              deleteKeyInMap(key);
            }
            if(deleteValueInMap){ // If deleteValueInMap is referenced
      ...
      else if(keyEqual !=0 && valueEqual == 0){ // if keyEqual is but valueEqual is not referenced
          if(keyEqual(key,temp.get(i)->key) && value == temp.get(i)->value){
            if(deleteKeyInMap){ // If deleteKeyInMap is referenced
              deleteKeyInMap(key);
            }
            if(deleteValueInMap){ // If deleteValueInMap is referenced
      ...
      else if(value == temp.get(i)->value && key == temp.get(i)->key){ // both keyEqual and valueEqual are not referenced
            if(deleteKeyInMap){ // If deleteKeyInMap is referenced
              deleteKeyInMap(key);
            }
            if(deleteValueInMap){ // If deleteValueInMap is referenced
...
bool xMap<K, V>::containsKey(K key) {
  ...
  for(auto list : temp){
      if(keyEqual){
        if(keyEqual(key,list->key)) return true;
      }
...
bool xMap<K, V>::containsValue(V value) {
  ...
  for(int idx = 0; idx < capacity; idx++){
    DLinkedList<Entry* > temp = this->table[idx];
    for(auto list : temp){
        if(valueEqual){
          if(valueEqual(value,list->value)) return true;
        }
...
DLinkedList<K> xMap<K, V>::keys() {
  // TODO YOUR CODE IS HERE
  DLinkedList<K> KeyArray;
  for(int idx = 0; idx < capacity; idx++){
    DLinkedList<Entry* > temp = this->table[idx];
    for(auto list : temp){
        KeyArray.add(list->key);
    }
  }
  return KeyArray;
}
...
DLinkedList<V> xMap<K, V>::values() {
  // TODO YOUR CODE IS HERE
  DLinkedList<V> ValueArray;
 for(int idx = 0; idx < capacity; idx++){
    DLinkedList<Entry* > temp = this->table[idx];
    for(auto list : temp){
        ValueArray.add(list->value);
    }
  }
  return ValueArray;
}
